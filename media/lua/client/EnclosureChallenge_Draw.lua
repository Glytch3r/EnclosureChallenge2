...
function EnclosureChallenge.GUI()
    ...
    local alpha = EnclosureChallenge.alphaGUI or 1
    ...
    local badColor = SandboxVars.EnclosureChallengeGUI.BadColor   or {r=1,g=0,b=0,a=1}
    if isOutOfBounds and isChallenger then
        col = EnclosureChallenge.parseColor( badColor)
    end
    ...
    local optSizes = EnclosureChallenge.FontSize or 1
...
-----------------------            ---------------------------
--[[
EnclosureChallenge.OptionUI = {}
function EnclosureChallenge.OptionUI.create()
    if EnclosureChallenge.OptionUI.window then return end
    ...
    local win = ISCollapsableWindow:new(100, 100, width, height)
    win.title = "Challenge GUI Settings"
    win.pin = true
    win:initialise()
    win:addToUIManager()
    ...
    function win:onClose()
        ISCollapsableWindow.onClose(self)
        EnclosureChallenge.OptionUI.window = nil
    end

    EnclosureChallenge.OptionUI.window = win

    local margin = 10
    local sliderW = width - (margin * 2)
    local labelH = 18
    local sliderH = 20
    local y = 28

    local function addSlider(label, varName, val, min, max)
        local lbl = ISLabel:new(margin, y, labelH, label, 1, 1, 1, 1, UIFont.Small, true)
        win:addChild(lbl)
        y = y + labelH

        local slider = ISSliderPanel:new(margin, y, sliderW, sliderH, nil, function(_, newVal)
            settings[varName] = math.floor(newVal)
        end)
        slider:setDoButtons(false)
        slider:setValues(min, max, 1, 10)
        slider:setCurrentValue(val or min)
        win:addChild(slider)

        y = y + sliderH + 4
    end

    addSlider("X Offset", "xOffset", settings.xOffset or 125, 0, 500)
    addSlider("Y Offset", "yOffset", settings.yOffset or 125, 0, 500)
    addSlider("Text Gap", "textGap", settings.textGap or 5, 0, 100)
