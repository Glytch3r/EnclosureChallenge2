...
function EnclosureChallenge.onYes(isQuit, isRemote)
    ...
    if EnclosureChallenge.isShouldAnnounce() then
        ...
        local posStr = (SandboxVars.EnclosureChallenge.CoordNotif and encStr) and ": [" .. encStr .. "]" or ""
        local key = isQuit and "ContextMenu_EnclosureChallenge_ChallengeWithdrawn" or "ContextMenu_EnclosureChallenge_ChallengeAccepted"
        local msg = string.format("%s %s %s", user, getText(key), posStr)
...
function EnclosureChallenge.onNo(isQuit, isRemote)
    ...
    if not isQuit and isRemote then
...
--[[
function EnclosureChallenge.ConfirmDialog(pl, text, title, isQuit, isRemote, unlockTarg)
    pl = pl or getPlayer(); if not pl or not text then return end
    if not isQuit and EnclosureChallenge.isChallenger() then return end
